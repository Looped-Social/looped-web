        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          {onForgotPassword && (
            <button
              ...
              disabled={isBusy}
              className="text-left text-xs font-semibold text-brand transition hover:text-brand/90 disabled:cursor-not-allowed disabled:opacity-60"
            >
              Forgot password?
            </button>
          )}
          ...
          {resetMessage && (
            <p className={`text-sm ${resetTone === "error" ? "text-brand" : "text-text-secondary"}`}>
              {resetMessage}
            </p>
          )}
